...
class PerfumeBusinessEngine:
    ...
    def calculate_global_fit(self, molecules: List[Dict]) -> Dict:
        ...
        for m in molecules:
            ...
            trad_use = m.get('traditional_use', db_data.get('traditional_use', ''))
            if trad_use:
        ...
        for region, criteria in self.MARKETS.items():
            ...
            keyword_matches = sum(1 for kw in criteria['keywords'] if kw.lower() in full_text)
    ...
    def estimate_financials(self, molecules: List[Dict], tech_score: float, neuro_score: float) -> Dict:
        ...
        if neuro_score > 0.7: multiplier += 3.0 
    ...
    def validate_ifra(self, molecules: List[Dict]) -> Dict:
        ...
        for m in molecules:
            ...
            if skin_exposure > limit:
                is_legal = False
                severity = (skin_exposure - limit) / limit if limit > 0 else 100.0
                logs.append(f"IFRA Violation: {m['name']} ({skin_exposure:.1%} > Limite {limit:.1%})")
