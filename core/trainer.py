...
class ModelTrainer:
    ...
    def maybe_retrain(self, buffer, threshold=50, force=False):
        """
        Retreina se houver dados novos suficientes OU se for forçado (feedback humano).
        """
        if force and buffer.size() > 0:
            avg_loss = self.retrain(buffer, epochs=5)
            print(f"[TRAINER] Treino forçado concluído. Loss: {avg_loss:.4f}")
            return True
    ...
    def retrain(self, buffer, epochs=10):
        ...
        loader = DataLoader(
            data_list, batch_size=self.batch_size, shuffle=True)
