...
# Since rdkit is mocked in conftest, we can access it here via sys.modules
...
def test_evaluate_blend_basic(chemistry_engine, sample_molecules):
    # Ensure sample molecules have necessary data
...
def test_calculate_projection(chemistry_engine, sample_molecules):
    ...
    proj = chemistry_engine._calculate_projection(sample_molecules, vapor_pressures, logps)
...
def test_calculate_longevity(chemistry_engine, sample_molecules):
    ...
    long_score = chemistry_engine._calculate_longevity(sample_molecules, vapor_pressures, logps)
...
def test_estimate_vapor_pressure(chemistry_engine):
    # Test for water-like boiling point
...
def test_detect_chemical_risks(chemistry_engine):
    # This test relies on mocked rdkit behavior.
    # Since we mocked rdkit completely, Chem.MolFromSmiles returns a MagicMock.

    m1 = {"name": "Test1", "smiles": "C=O", "olfactive_family": "Floral"}
    m2 = {"name": "Test2", "smiles": "CN", "olfactive_family": "Floral"}

    # We configure the return value of MolFromSmiles to be a mock that returns True for HasSubstructMatch
    mock_mol = MagicMock()
    mock_mol.HasSubstructMatch.return_value = True

    # We need to ensure Chem is the one used in chemistry.py
    # modifying the mock in sys.modules should affect the one imported in chemistry.py
    Chem.MolFromSmiles.return_value = mock_mol

    risks, penalty = chemistry_engine._detect_chemical_risks([m1, m2])

    # If HasSubstructMatch always returns True, both molecules will be seen as Aldehydes AND Amines.
    # Because code checks:
    # if mol.HasSubstructMatch(aldehyde_patt): ...
    # if mol.HasSubstructMatch(amine_patt): ...

    # So both lists (aldehydes_found, amines_found) will be populated.
    # This should trigger the Schiff Base risk.
